...
OUT_DIR = Path("reports") / "v42_candidate_a_validation"
DATASETS = (
...
class RecordingStateEstimator:
    def __init__(self, *, k_tau: float) -> None:
        ...
        self.mapper = MinimalStateEstimator(
            config=StateEstimatorConfig(k_tau=k_tau),
        )
...
def main() -> None:
    ...
    csv_path = OUT_DIR / "v42_candidate_a_validation_comparison.csv"
    md_path = OUT_DIR / "v42_candidate_a_validation_comparison.md"
...
def validate_dataset(*, asset: str, window: str, prices: pd.Series, fee_rate: float) -> list[dict[str, Any]]:
    ...
    rows = [
        run_control_strategy(asset=asset, window=window, fee_rate=fee_rate, prices=prices, strategy_name="v4.1_default", k_tau=1.0),
        run_control_strategy(asset=asset, window=window, fee_rate=fee_rate, prices=prices, strategy_name="v4.2_candidate_A", k_tau=5.0),
        ...
        summarize_backtest_result(
            ...
            window=window,
            fee_rate=fee_rate,
            strategy_name="controller_buy_and_hold",
            result=run_controller_buy_and_hold_benchmark(prices, config=benchmark_config),
            state_frame=None,
        ),
        summarize_backtest_result(
            asset=asset,
